...
async function getJson(url, options = {}, label = 'request', attempt = 0) {
  let res;
  try {
    res = await fetchWithTimeout(url, { ...options, headers: { Accept: 'application/json', ...options.headers } });
  } catch (error) {
    // network error / timeout: retry once, then report as upstream
    if (attempt === 0) return getJson(url, options, label, 1);
    throw httpError(502, 'upstream', `${label}: ${error.name === 'AbortError' ? 'timeout' : error.message}`);
  }
  if (res.status >= 500 && attempt === 0) {
    await new Promise(resolve => setTimeout(resolve, 800));
    return getJson(url, options, label, 1);
  }
